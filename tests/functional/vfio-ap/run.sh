...
kata_base="/opt/kata"
runtime_rs_bin_base="${kata_base}/runtime-rs/bin"
runtime_rs_config_base="${kata_base}/share/defaults/kata-containers/runtime-rs"
...
setup_config_file() {
    ...
    local action=$2
    local replacement=${3:-}
    local kata_config_file=${4:-$(kata-runtime env --json | jq -r '.Runtime.Config.Path')}
...
show_config_file() {
    local kata_config_file=${1:-$(kata-runtime env --json | jq -r '.Runtime.Config.Path')}
...
setup_hotplug() {
    ...
    if [ "${runtime}" == "runtime" ]; then
        ...
        setup_config_file "vfio_mode" "replace" "vfio" "${runtime_rs_config_base}/configuration.toml"
        show_config_file "${runtime_rs_config_base}/configuration.toml"
    else
...
configure_containerd_for_runtime_rs() {
    ...
    sudo rm -f /usr/local/bin/containerd-shim-kata-qemu-runtime-rs-v2 \
        ${runtime_rs_config_base}/configuration.toml
    if [ ! -f ${runtime_rs_bin_base}/containerd-shim-kata-v2 ]; then
        echo "${runtime_rs_bin_base}/containerd-shim-kata-v2 not found" >&2
        exit 1
    fi
    if [ ! -f ${runtime_rs_config_base}/configuration-qemu-runtime-rs.toml ]; then
        echo "${runtime_rs_config_base}/configuration-qemu-runtime-rs.toml not found" >&2
        exit 1
    fi
    sudo ln -sf ${runtime_rs_bin_base}/containerd-shim-kata-v2 \
        /usr/local/bin/containerd-shim-kata-qemu-runtime-rs-v2
    sudo ln -sf ${runtime_rs_config_base}/configuration-qemu-runtime-rs.toml \
        ${runtime_rs_config_base}/configuration.toml
