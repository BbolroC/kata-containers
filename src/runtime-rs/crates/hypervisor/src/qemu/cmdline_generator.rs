...
impl ToQemuParams for Memory {
    fn qemu_params(&self) -> Result<Vec<String>> {
        ...
        if self.size.trailing_zeros() < 19 {
            return Err(anyhow!("bad memory size (must be given in whole megabytes): {}", self.size));
        }
...
impl DeviceVhostUserFsPci {
    ...
    fn set_queue_size(&mut self, queue_size: u64) -> &mut Self {
        if queue_size <= 1024 && queue_size.is_power_of_two() {
            ...
            warn!(sl!(), "bad vhost-user-fs-pci queue_size (must be power of two): {}, ignoring", queue_size);
        }
        self
    }
